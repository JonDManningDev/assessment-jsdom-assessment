...
function renderContact(contact) {
  const { id, email, picture, address: { street, suite, city, zipcode }, phone, website } = contact;
  ...
  <div class="card" data-id="${id}">
    ...
    <div class="info">
      <span class="name big">${personName}</span>
...
const contactsContent = contacts.map(renderContact).join("");
contactSection.innerHTML = contactsContent;
...
function loadCities(contacts) {
  ...
  const cities = [...new Set(contacts.map((contact) => contact.address.city))];

  const cityOptions = cities.map((city) => `<option value="${city}">${city}</option>`);
  cityOptions.unshift(`<option value="0">-- Select a city --</option>`);

  const filterOptions = document.querySelector("#filterOptions");
  filterOptions.innerHTML = cityOptions.join("");
};
...
function deleteButtonHandler() {   
  ...
  contactsSection.addEventListener("click", (event) => {
    if (event.target && event.target.classList.contains("deleteBtn")) {
      const contactCard = event.target.parentNode;
      deleteContact(contactCard.dataset.id);
      loadCities(contacts);
      render(contacts);
    };
  });
